...
$(document).ready( () => {
            ...
            },
            {
                ...
                render: (data, type, row) => {
                    ...
                    switch(data.toLowerCase()){
                        case 'pending':
                            renderStatus = "<span class='badge badge-warning' style='font-size:16pt;'> รอการตรวจสอบ </span>"
                            break;
                        case 'approve':
    ...
    "reportx_product" : [
        ...
        },
        {
            ...
            render: (data, type, row) => {
                return (`
                    <div class='text-right'>${addComma(row['product_price'])}</div>
                `)
            }
        },
        {
        ...
        },
        {
            data: 'product_status',
            render: (data, type, row) => {
                let statBox = data  === true ? '<span class="badge badge-success">ใช้งาน</span>' : '<span class="badge badge-danger">ยกเลิกใช้งาน</span>' ;
                return (`
                    ${statBox}
                `)
            }
    ...
    var tablesx = $('#' + gClass).DataTable({
        ajax: {
            method: 'GET',
            url: 'http://localhost:3000/backendx/api/' + gClass.replace('_', "/") + '/table_data',
            data: function(d){
    ...
    $('body').on('click', '.btn-add', async(e) => {
        ...
        let urlPost = editbyid == undefined ? gUrl + '/api/' + gClass + '/' + action : gUrl + '/api/' + gClass + '/' + action +'/' + editbyid;
        console.log(urlPost);
