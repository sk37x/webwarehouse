...
$("#ajaxModal, #ajaxLargeModal").on("show.bs.modal", function (e) {
    ...
    let { used } = e.relatedTarget.dataset
...
$(document).ready(() => {
		...
		columns: [
			{
				...
				render: (data, type, row) => {
					return `
                        <a data-toggle="modal" data-target="#ajaxLargeModal" href="api/product/${row["_id"]}/${userRole}/${row['used']}">${row["product_name"]}</a>
                    `;
				},
			},
			{
				...
				render: (data, type, row) => {
					return `
                        <a data-toggle="modal" data-target="#ajaxLargeModal" href="api/product/${row["_id"]}/${userRole}/${row['used']}">${row["product_code"]}</a>
                    `;
				},
			},
			{
			...
			},
			{
				...
				render: (data, type, row) => {
                    ...
                    <a
                        class="btn btn-outline-dark btn-sm mr-7 fas fa-edit"
                        href="api/product/${data}/${userRole}/${row["used"]}"
	...
	$("body").on("click", ".btn-add-product", async (e) => {
		...
		if (formProduct.product_name.value.length === 0) {
			let spanAlert = $(
				'span[data-from="' + formProduct.product_name.name + '"]'
			);
		...
		if (formProduct.product_category.value.length === 0) {
			let spanAlert = $(
				'span[data-from="' + formProduct.product_category.name + '"]'
			);
			spanAlert.addClass("error");
		...
		if (formProduct.product_unitCount.value.length === 0) {
			let spanAlert = $(
				'span[data-from="' + formProduct.product_unitCount.name + '"]'
			);
			spanAlert.addClass("error");
